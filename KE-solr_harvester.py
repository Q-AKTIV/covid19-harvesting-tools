...
print(solr.ping())  ## check ob die verbindung funktioniert
'''
terms = ['Ebolavirus',
         'Betacoronavirus',
         'Zika Virus',
...
for year in years:
    for term in terms:
        anzahl = solr.search('MESH:"{}" AND PUBLYEAR:"{}"'.format(term,year)).hits
        print(f'{year}{term}:', anzahl)
    print('XXXX', year , 'XXXXXX')
...
#def get_info(jahr, hh):
 #   payload = {"PUBLYEAR[* TO *]": jahr}
  #  print('PAYLOAD', payload)
   # r = requests.get('http://134.95.56.177:8080/solr/default/select?wt=json&q=(DOI:10.1364%2FOE.20.000816)', params=payload)
    #print('r:', r.url)
    #print('r:', r.request)
    #print(dir(r))
...
#http://z3950.zbmed.de:6210/livivo?version=1.1&operation=searchRetrieve&query=dc.identifier=
##curl "http://z3950.zbmed.de:6210/livivo?version=2.0&operation=searchRetrieve&query=dc.identifier=10.1371/journal.pone.0157620&&recordSchema=xml&x-username=zbmintern" -O "tmp.xml"